...
namespace SudokuCollective.Heroku
{
    public static class HerokuProxy
    {
        ...
        public static async Task<bool> UpdateHerokuPostgresConnectionStringAsync(bool isStaging, ILogger logger)
        {
            string herokuApp, herokuPostgresApp;

            if (isStaging)
            {
                herokuApp = Environment.GetEnvironmentVariable("HEROKU:STAGING:HEROKUAPP")!;
                herokuPostgresApp = Environment.GetEnvironmentVariable("HEROKU:STAGING:HEROKUPOSTGRES")!;
            }
            else
            {
                herokuApp = Environment.GetEnvironmentVariable("HEROKU:PROD:HEROKUAPP")!;
                herokuPostgresApp = Environment.GetEnvironmentVariable("HEROKU:PROD:HEROKUPOSTGRES")!;
            }

            using var httpClient = new HttpClient();

            #region Obtain Heroku Postgres Settings
            var getUrl = string.Format(configUrl, herokuPostgresApp);
            ...
            #region Get Postgres Connection Values
            var postgresConfigs = await getResponse.Content.ReadFromJsonAsync<List<ConfigVar>>();
            var database_url = postgresConfigs.FirstOrDefault().Value;
            #endregion

            #region Update Heroku App Postgres Connection
            var patchUrl = string.Format(configVarsUrl, herokuApp);

            using var patchRequest = new HttpRequestMessage(HttpMethod.Patch, patchUrl);

            patchRequest.Headers.Add("Accept", "application/vnd.heroku+json; version=3");
            patchRequest.Headers.Add("Content-Type", "application/json");
            patchRequest.Headers.Add("Authorization", string.Format("Bearer {0}", Environment.GetEnvironmentVariable("HEROKUCREDENTIALS:TOKEN")));
